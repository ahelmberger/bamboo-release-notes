...
function createBasicAuthHeader(name, pass) {
  var separator = pass ? ':' : '';
  return 'Basic ' + new Buffer(name + separator + pass).toString('base64');
}
...
module.exports = function (options) {
  var url = options.bambooServer +
            ...
            '/' +
            options.buildNumber +
            '?os_authType=basic&expand=changes.change.files';

  var headers = {
    'Accept': 'application/json',
    'Authorization': createBasicAuthHeader(options.bambooUserName, options.bambooPassword)
  };

  return fetch(url, headers).then(handleResponse).then(createReleaseNotes);
};
